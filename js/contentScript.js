...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.data) {
    chrome.storage.local.set({ 'shopeeData': request.data }, (result) => {
      //? Notify that we saved.
      console.log('Settings saved');
    });
    console.log(request.data)
    $('#kt_subheader_total').text(`${JSON.stringify(request.data.totalItemsData.length)} Total`)
    let totalItemsData = request.data.totalItemsData
    let shopAccount = request.data.shopAccount
    let shopeeIndex = {
      init: () => {
        let e, t;
        e = request.data.totalItemsData,
